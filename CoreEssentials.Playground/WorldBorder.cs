...
public class WorldBorder : Entity
{
    ...
    private void CreateWorldBorder()
    {
        ...
        _borderBodies[0] = PhysicsEngine.Instance.CreateBody(new Vector2(_position.X, _position.Y + _size.Y / 2), 0, BodyType.Static);
        _borderBodies[0].CreateRectangle(1, _size.Y, 1, Vector2.Zero);

        // Create the right border
        _borderBodies[1] = PhysicsEngine.Instance.CreateBody(new Vector2(_position.X + _size.X, _position.Y + _size.Y / 2), 0, BodyType.Static);
        _borderBodies[1].CreateRectangle(1, _size.Y, 1, Vector2.Zero);

        // Create the top border
        _borderBodies[2] = PhysicsEngine.Instance.CreateBody(new Vector2(_position.X + _size.X / 2, _position.Y), 0, BodyType.Static);
