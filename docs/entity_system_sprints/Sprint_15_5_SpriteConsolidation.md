...
- [ ] **T1: Rename `AnimatedSprite` → `Sprite`, fold in old `Sprite` (2 pts)** ⭐ User-facing / 🔒 Internal
...
- [ ] **T2: Content XML schema + migration (0.5 pt)** ⭐ User-facing
...
- [ ] **T3: Add `AnimationComponent` (1.5 pts)** ⭐ User-facing
...
- [ ] **T4: Wire `Entity.GetSize()` fallback chain (0.5 pt)** 🔒 Internal
  - `SpriteComponent` → `AnimationComponent` → `Vector2.Zero`

- [ ] **T5: Register `AnimationComponent` in `EntitySerializer` (0.25 pt)** 🔒 Internal
  - `Register<AnimationComponent>("AnimationComponent")`

- [ ] **T6: Refactor playground entities to components (0.5 pt)** ⭐ User-facing
...
- [ ] **T7: Write unit tests (0.5 pt)** 🔁 Validation
...
- [ ] **T8: Documentation + migration guide (0.75 pt)** 📚 User-facing
...
- [ ] Single `Sprite` type handles both static (texture2d) and animated (spritesheet) sources
- [ ] `AnimatedSprite` class removed; all references updated
- [ ] No red debug rectangle drawn on sprites by default
- [ ] `AnimationComponent` drives named animations into a `SpriteComponent`
- [ ] Base `Entity.GetSize()` works for sprite and animated entities without overrides
- [ ] `CharacterEntity` and `AnimatedCharacterEntity` use components (no sprite overrides)
- [ ] Project builds cleanly — **0 errors, 0 warnings**
- [ ] All existing tests pass + new sprite/animation tests added
- [ ] Migration guide documents the breaking rename + XML schema change
...
| `SpriteTests.cs` / `AnimationComponentTests.cs` | New | 🔒 Internal | Unit tests |
